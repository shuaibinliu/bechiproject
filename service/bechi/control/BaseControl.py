...
class BaseController:
    ...
    def _commision_preview(self, *args, **kwargs):
        ...
        计算最低比例
        :param price:  价格
        :param bechi_rate:  平台最低比
        :param bechi_and_user_rate: 供应商让利比
        :return:
        """
        price = Decimal(str(kwargs.get('price')))  # 299
        bechi_rate = Decimal(str(kwargs.get('bechi_rate')))  # 5
        bechi_and_user_rate = Decimal(str(kwargs.get('bechi_and_user_rate'))) / 100  # 0.2
        current_user_rate = Decimal(str(kwargs.get('current_user_rate'))) / 100  # 0.5
        bechi_rate = Decimal(bechi_rate) / 100  # 0.05
        user_rate = bechi_and_user_rate - bechi_rate  # 0.15
