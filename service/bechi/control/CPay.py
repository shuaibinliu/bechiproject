...
class CPay():
    ...
    def _caculate_commsion(self, user, up1, up2, up3, commision, order_part, is_act=False):
        ...
        if is_act:
            ...
            UCstatus = None
        default_level1commision, default_level2commision, default_level3commision, default_bechicommision = json.loads(
        ...
        bechi_and_user_rate = Decimal(str(order_part.SkudevideRate or 0))

        if not bechi_and_user_rate:
            sku = ProductSku.query.filter(ProductSku.SKUid == OrderPart.SKUid).first()
            if sku:
                bechi_and_user_rate = Decimal(str(sku.SkudevideRate or 0))
        if not bechi_and_user_rate:
            bechi_and_user_rate = default_bechicommision
        bechi_and_user_rate = Decimal(bechi_and_user_rate) / 100
        # 平台固定抽成
        bechi_rate = Decimal(default_bechicommision) / 100
        bechi_commision = order_part.OPsubTotal * bechi_rate   # 平台获得, 是总价的的百分比
        user_commision = order_part.OPsubTotal * bechi_and_user_rate - bechi_commision  # 用户获得, 是总价 - 平台获得
        # user_rate = bechi_and_user_rate - bechi_rate  # 用户的的比例
        # 用户佣金
        commision_for_supplizer = order_part.OPsubTotal * (Decimal('1') - bechi_and_user_rate)   #  给供应商的钱   总价 * ( 1 - 让利 )
        ...
        if up3_base:
            ...
            db.session.add(commision_account)
        bechi_remain = user_commision + bechi_commision
        # 优惠券计算
        ...
        if order_coupon:
            if order_coupon.SUid:
                ...
                bechi_remain -= (Decimal(order_part.OPsubTotal) - Decimal(order_part.OPsubTrueTotal))
        ...
        if suid:
            ...
            bechi_remain += commision_for_supplizer
        # 平台剩余佣金
        commision_account = UserCommission.create({
            'UCid': str(uuid.uuid1()),
            'OMid': order_part.OMid,
            'OPid': order_part.OPid,
            'UCcommission': bechi_remain,
            'USid': '0',
        ...
        db.session.add(commision_account)
        current_app.logger.info('平台获取: {}'.format(bechi_remain))
    ...
    def get_preview_commision(self):
        ...
        default_bechicommision = level_commision[-1]
        ...
        for sku in skus:
            ...
            bechi_and_user_rate = self._current_commission(sku_instance.SkudevideRate, getattr(supplizer, 'SUbaseRate', ''), default_bechicommision)
            bechi_and_user_rate = Decimal(bechi_and_user_rate) / 100
            # 平台固定抽成
            bechi_rate = Decimal(default_bechicommision) / 100
            bechi_commision = order_price * bechi_rate  # 平台获得, 是总价的的百分比
            user_commision = order_price * bechi_and_user_rate - bechi_commision  # 用户获得, 是总价 - 平台获得
            # 正常应该获得佣金
            up1_base = up2_base = 0
            if up1 and up1.USlevel > 1:
